...
def users():
    ...
    while True:
        ...
        fake_datetime = faker.date_time_between(
...
def invoices():
    ...
    for user in db.session.query(User).all():
        for i in range(randint(1, 12)):
            fake_created_on = faker.date_time_between(
                start_date="-1y", end_date="now"
            ).strftime("%s")
            created_on = datetime.utcfromtimestamp(float(fake_created_on)).strftime(
                "%Y-%m-%dT%H:%M:%S Z"
            )

            fake_period_start_on = faker.date_time_between(
                start_date="now", end_date="+1y"
            ).strftime("%s")
            period_start_on = datetime.utcfromtimestamp(
                float(fake_period_start_on)
            ).strftime("%Y-%m-%d")

            fake_period_end_on = faker.date_time_between(
                start_date=period_start_on, end_date="+14d"
            ).strftime("%s")
            period_end_on = datetime.utcfromtimestamp(
                float(fake_period_end_on)
            ).strftime("%Y-%m-%d")

            fake_exp_date = faker.date_time_between(
                start_date="now", end_date="+2y"
            ).strftime("%s")
            exp_date = datetime.utcfromtimestamp(float(fake_exp_date)).strftime(
                "%Y-%m-%d"
            )
    ...
    for user in db.session.query(User).all():
        for i in range(randint(10, 20)):
            ...
            payout = Bet.determine_payout(
                float(app.config["DICE_ROLL_PAYOUT"][str(guess)]), is_winner
            )
